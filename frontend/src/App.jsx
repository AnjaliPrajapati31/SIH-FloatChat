  ...
  function handleCommand(cmd) {
    ...
    if (t.includes("compare floats") || t.includes("compare buoys")) {
      ...
      if (buoyMatches && buoyMatches.length >= 2) {
        ...
        const validBuoys = [buoy1, buoy2].filter(id => availableBuoys.includes(id))
    ...
    // Time comparison for single buoy
    if (t.includes("comparison")) {
      ...
      if (t.includes("pressure")) {
        setChartMode("pressure")
        setCompareMode(true)
        return "Showing pressure comparison between different time periods."
      }
      if (t.includes("depth")) {
        setChartMode("depth")
        setCompareMode(true)
        return "Showing depth comparison between different time periods."
      }
      setCompareMode(true)
...
• "comparison [parameter]" - compare time periods for same buoy
