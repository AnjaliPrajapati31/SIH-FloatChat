...
export default function ChartView({
  ...
  if (compareTwoFloats && selectedBuoys.length >= 2) {
    ...
    if (buoy1Data && buoy2Data) {
      ...
      if (buoy1Cycles.length > 0 && buoy2Cycles.length > 0) {
        ...
        chartData = {
          ...
          datasets: [
            {
              label: `Buoy ${buoy1Id} - ${new Date(
                timestamp1
              ).toLocaleDateString()}`,
              data: cycle1.measurements
                .slice(0, maxMeasurements)
                .map((m) => getParam(m)),
            ...
            {
              label: `Buoy ${buoy2Id} - ${new Date(
                timestamp2
              ).toLocaleDateString()}`,
              data: cycle2.measurements
                .slice(0, maxMeasurements)
                .map((m) => getParam(m)),
    ...
    if (uniqueDates.length > 0) {
      const firstCycleData = filteredData
        .filter((d) => d.date === uniqueDates[0])
        .slice(0, 100);
      ...
      if (firstCycleData.length > 0) {
        chartData = {
          ...
          datasets: [
            {
              label: `Buoy ${firstCycleData[0].id} - ${new Date(
                uniqueDates[0]
              ).toLocaleDateString()} ${mode}`,
  ...
  const getChartTitle = () => {
    if (compareTwoFloats && selectedBuoys.length >= 2) {
      ...
      } Comparison - Buoys ${selectedBuoys[0]} vs ${selectedBuoys[1]}`;
    } else if (compare) {
      const buoyId =
        selectedBuoys.length > 0
          ? selectedBuoys[0]
          : originalData?.[0]?.id || "Unknown";
      return `${
        mode.charAt(0).toUpperCase() + mode.slice(1)
      } Time Comparison - Buoy ${buoyId}`;
    } else {
      const buoyId =
        selectedBuoys.length > 0
          ? selectedBuoys[0]
